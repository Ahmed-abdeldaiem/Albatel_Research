...
// =============================================================================
// BlogTopics — أربعة محاور تُشكّل المدونة
// نفس نمط بطاقات `PublicationsQualities`: أيقونة متدرّجة، هالة عند الهافر،
// وخط سفلي متحرّك.
// =============================================================================
...
export function BlogTopics() {
  ...
  const topics: Topic[] = [
    {
      title: t("media.blog.t1T"),
      desc: t("media.blog.t1D"),
      icon: "fa-flask",
      tone: "from-brand-500 to-cyan-500",
    },
    {
      title: t("media.blog.t2T"),
      desc: t("media.blog.t2D"),
      icon: "fa-book-open-reader",
      tone: "from-amber-500 to-orange-500",
    },
    {
      title: t("media.blog.t3T"),
      desc: t("media.blog.t3D"),
      icon: "fa-language",
      tone: "from-emerald-500 to-teal-500",
    },
    {
      title: t("media.blog.t4T"),
      desc: t("media.blog.t4D"),
      icon: "fa-chart-line",
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {topics.map((topic, i) => (
            ...
            >
              <div
                className={`pointer-events-none absolute -top-16 -end-16 h-40 w-40 rounded-full bg-gradient-to-br ${topic.tone} opacity-0 blur-3xl transition-opacity duration-500 group-hover:opacity-20`}
                aria-hidden
              />

              <div
                className={`relative inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br ${topic.tone} text-white shadow-md transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3`}
              >
                <i className={`fa-solid ${topic.icon} text-xl`} aria-hidden />
              </div>
              ...
              <span
                className={`relative mt-2 block h-0.5 w-10 origin-start scale-x-0 bg-gradient-to-r ${topic.tone} transition-transform duration-500 group-hover:scale-x-100`}
                aria-hidden
              />
